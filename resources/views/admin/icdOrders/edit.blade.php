...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.icdOrder.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.icd-orders.update", [$icdOrder->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="order_number">{{ trans('cruds.icdOrder.fields.order_number') }}</label>
                <input class="form-control {{ $errors->has('order_number') ? 'is-invalid' : '' }}" type="text" name="order_number" id="order_number" value="{{ old('order_number', $icdOrder->order_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('icd_cm') ? 'is-invalid' : '' }}" type="text" name="icd_cm" id="icd_cm" value="{{ old('icd_cm', $icdOrder->icd_cm) }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\IcdOrder::CATEGORY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('category') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="category_{{ $key }}" name="category" value="{{ $key }}" {{ old('category', $icdOrder->category) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('short_desc') ? 'is-invalid' : '' }}" type="text" name="short_desc" id="short_desc" value="{{ old('short_desc', $icdOrder->short_desc) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('long_desc') ? 'is-invalid' : '' }}" name="long_desc" id="long_desc">{!! old('long_desc', $icdOrder->long_desc) !!}</textarea>
