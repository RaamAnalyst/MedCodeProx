...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.icdPcsOrder.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.icd-pcs-orders.update", [$icdPcsOrder->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="pcs_order_number">{{ trans('cruds.icdPcsOrder.fields.pcs_order_number') }}</label>
                <input class="form-control {{ $errors->has('pcs_order_number') ? 'is-invalid' : '' }}" type="text" name="pcs_order_number" id="pcs_order_number" value="{{ old('pcs_order_number', $icdPcsOrder->pcs_order_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('icd_pcs_code') ? 'is-invalid' : '' }}" type="text" name="icd_pcs_code" id="icd_pcs_code" value="{{ old('icd_pcs_code', $icdPcsOrder->icd_pcs_code) }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\IcdPcsOrder::PCS_CATEGORY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('pcs_category') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="pcs_category_{{ $key }}" name="pcs_category" value="{{ $key }}" {{ old('pcs_category', $icdPcsOrder->pcs_category) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pcs_short_desc') ? 'is-invalid' : '' }}" type="text" name="pcs_short_desc" id="pcs_short_desc" value="{{ old('pcs_short_desc', $icdPcsOrder->pcs_short_desc) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('pcs_long_desc') ? 'is-invalid' : '' }}" name="pcs_long_desc" id="pcs_long_desc">{{ old('pcs_long_desc', $icdPcsOrder->pcs_long_desc) }}</textarea>
